...
export default function HomePage() {
  ...
  const [announcementInput, setAnnouncementInput] =
    useState("")

  const [announcement, setAnnouncement] =
    useState("")

  const [kickUsername, setKickUsername] =
    useState("")

  const [creditUsername, setCreditUsername] =
    useState("")

  const [creditAmount, setCreditAmount] =
    useState("")

  const [removeUsername, setRemoveUsername] =
    useState("")
  ...
  const [reportingWebsite, setReportingWebsite] =
    useState<Website | null>(null)

  const [bugReason, setBugReason] = useState("")

  const [showMiniGames, setShowMiniGames] =
    useState(true)
  ...
  const [siteLocked, setSiteLocked] =
    useState(false)

  const [showProfile, setShowProfile] =
    useState(false)

  const [showAdminBadge, setShowAdminBadge] =
    useState(false)

  const [showUserStatus, setShowUserStatus] =
    useState(false)

  const [showTrendingApps, setShowTrendingApps] =
    useState(false)

  const [showComments, setShowComments] =
    useState(false)

  const [showRecentlyVisited, setShowRecentlyVisited] =
    useState(false)
  ...
  const [globalChatCommands, setGlobalChatCommands] =
    useState(false)
  ...
  const [featuredApp, setFeaturedApp] =
    useState<Website | null>(null)

  const [showSecretLinks, setShowSecretLinks] =
    useState(false)

  const [secretApp, setSecretApp] =
    useState<Website>(secretApps[0])

  const [secretWebsite, setSecretWebsite] =
    useState<Website>(secretWebsites[0])
  ...
  useEffect(() => {
    const savedUser =
      localStorage.getItem(
        "generationAlphaUsername"
      )

    const savedUsers =
      localStorage.getItem(
        "generationAlphaUsers"
      )

    const savedFavorites =
      localStorage.getItem(
        "generationAlphaFavorites"
      )
    ...
    const savedBugReports =
      localStorage.getItem(
        "generationAlphaBugReports"
      )

    const savedPoints =
      localStorage.getItem(
        "generationAlphaUserPoints"
      )
    ...
    if (savedBugReports) {
      setBugReports(JSON.parse(savedBugReports))
    }
  ...
  function giveCredits() {
    ...
    addActivity(
      `Admin gave ${amount} UserPoints to ${creditUsername}`
    )
  ...
  function removeCredits() {
    ...
    addActivity(
      `Admin removed ${amount} UserPoints from ${removeUsername}`
    )
  ...
  function unlockWebsite(
    ...
    if (
      unlockedItems.includes(website.name)
    )
      return

    if (userPoints < cost) {
      alert(
        `You need ${cost.toLocaleString()} UserPoints`
      )
    ...
    addActivity(
      `${username} unlocked ${website.name}`
    )
  }
  ...
  function acceptBugReport(
    report: BugReport
  ) {
    alert(
      `Accepted bug report for ${report.websiteName}`
    )
  }
  ...
  function dismissBugReport(
    ...
    const updated = bugReports.filter(
      (report) =>
        report.id !== reportId
    )
  ...
  return (
    <main
      className={`min-h-screen px-6 py-12 ${backgroundClass}`}
    >
